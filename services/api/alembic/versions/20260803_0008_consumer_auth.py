...
def upgrade() -> None:
    # Earlier revisions used Base.metadata.create_all, so phone columns may
    # already exist when this revision runs on a fresh database.
    bind = op.get_bind()
    ...
    if "auth_otp_challenges" not in tables:
        ...
        op.create_index(
            "ix_auth_otp_challenges_channel", "auth_otp_challenges", ["channel"]
        )
        op.create_index(
            "ix_auth_otp_challenges_destination",
            "auth_otp_challenges",
            ["destination"],
        )
        op.create_index(
            "ix_auth_otp_challenges_expires_at",
            "auth_otp_challenges",
            ["expires_at"],
        )
        op.create_index(
