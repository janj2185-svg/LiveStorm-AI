 ...
 *
 * The card is the only opaque object on the lumen field, which makes it the
 * unambiguous focus. On a phone it loses its fill and fills the width: at 393px
 * a bordered card inside a bordered device is two frames doing one job, and the
 * inner one just steals horizontal space from the inputs.
 *
...
export function AuthScreen() {
  ...
  return (
    <div className="sy-screen sy-auth">
      {/* Outside the scroller, so the field stays put while the card moves. */}
      <div className="sy-lumen" />

      <div className="sy-auth__scroll">
        <div className="sy-auth__inner sy-screen__inner">
          <Surface className="sy-auth__card" elevation="raised" radius="2xl" padding="lg">
            <header className="sy-auth__head">
              <LogoMark size={44} tone="spectral" title="SYLORA" />
              ...
              {/* Refraction, used once: the seam between the brand and the form. */}
              <hr className="sy-refract-rule sy-auth__rule" />
            </header>
